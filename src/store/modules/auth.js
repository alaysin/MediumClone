...
export const mutationTypes = {
  ...
  loggingStart: '[auth] loggingStart',
  loggingSuccess: '[auth] loggingSuccess',
  loggingFailure: '[auth] loggingFailure',
...
const mutations = {
  ...
  [mutationTypes.registerSuccess](state, payload) {
    (state.isSubmitting = false), (state.currentUser = payload), (state.isLoggedIn = true);
  },

  [mutationTypes.registerFailure](state, payload) {
    (state.isSubmitting = false), (state.validationErrors = payload);
  },
  [mutationTypes.loggingStart](state) {
    state.validationErrors = null;
    state.isSubmitting = true;
  },
  [mutationTypes.loggingSuccess](state, payload) {
    state.isSubmitting = false;
    state.isLoggedIn = true;
    state.currentUser = payload;
  },
  [mutationTypes.loggingFailure](state, payload) {
    state.isSubmitting = false;
...
const actions = {
  ...
  [actionTypes.logging](context, credentials) {
    return new Promise((resolve) => {
      context.commit(mutationTypes.loggingStart);
      loggingApi
        .logging(credentials)
        .then((response) => {
          context.commit(mutationTypes.loggingSuccess, response.data.user);
          setItem('accessToken', response.data.user.token);
          resolve(response.data.user);
        })
        .catch((result) => {
          context.commit(mutationTypes.loggingFailure, result.response.data.errors);
        });
    });
  },
